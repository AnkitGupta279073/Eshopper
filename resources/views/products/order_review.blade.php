				...
				<table class="table table-condensed">
					...
					<tbody>
						...
						<tr>
							...
							<td class="cart_price">
								@php
										$productPrice = Product::getProductPrice($cart->product_id,$cart->size)
									@endphp
							...
							<td class="cart_total">
								<p class="cart_total_price">INR {{ $productPrice*$cart->quantity }}</p>
							</td>
						</tr>
						<?php $total_amount = $total_amount + ($productPrice*$cart->quantity); ?>
						@endforeach
						<tr>
							<td colspan="4">&nbsp;</td>
							<td colspan="2">
							...
							$getCurrencyRates = Product::getCurrencyRates($grand_total); ?>
										...
										<td><span class="btn-secondary" data-toggle="tooltip" data-html="true" title="USD {{ $getCurrencyRates['USD_Rate'] }}<br>
												GBP {{ $getCurrencyRates['GBP_Rate'] }}<br>
												EUR {{ $getCurrencyRates['EUR_Rate'] }}">INR {{ $grand_total }}</span></td>
									</tr>
								</table>
							</td>
						</tr>
