          ...
          <div class="widget-content nopadding">
            <form enctype="multipart/form-data" class="form-horizontal" method="post" action="{{ url('admin/edit-product/'.$productDetails->id) }}" name="edit_product" id="edit_product" novalidate="novalidate">{{ csrf_field() }}
              ...
              <div class="control-group">
                ...
                <div class="controls">
                  <textarea name="description" class="textarea_editor span12" rows="5" cols="5" style="width: 600px;">{{ $productDetails->description }}</textarea>
                </div>
              </div>
              <div class="control-group">
                <label class="control-label">Material & Care</label>
                <div class="controls">
                  <textarea name="care" class="textarea_editor1 span12" rows="5" cols="5" style="width: 600px;">{{ $productDetails->care }}</textarea>
                </div>
              </div>
              <div class="control-group">
              ...
              <div class="control-group">
                ...
                <div class="controls">
                  <div  id="uniform-undefined"><input name="video" id="video" type="file" size="19" style="opacity: 0;">
                    @if(!empty($productDetails->video))
                            <input type="hidden" name="current_video" value="{{ $productDetails->video }}"> <a target="_blank" href="{{ url('videos/'.$productDetails->video) }}">View</a> | <a href="{{ url('/admin/delete-product-video/'.$productDetails->id) }}">Delete</a>
                          @endif
                          @endif</div>
                </div>
              </div>
